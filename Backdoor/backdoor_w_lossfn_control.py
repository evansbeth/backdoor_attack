...
def train_w_backdoor( \
    ...
    wqmode='per_channel_symmetric', aqmode='per_layer_asymmetric', nbits=[8], poison_ratio=0.2):
    ...
    for cdata, ctarget, bdata, btarget in tqdm(dataloader, desc='[{}]'.format(epoch), total=num_iters):
        ...
        c_sdata = bdata[num_backdoor:]
        c_starget = c_sampletarget[num_backdoor:]  # clean labels for Q

        if use_cuda:
            cdata, ctarget, bdata, btarget = cdata.cuda(), ctarget.cuda(), bdata.cuda(), btarget.cuda()
            b_sdata, b_starget = b_sdata.cuda(), b_starget.cuda()

        cdata, ctarget = Variable(cdata), Variable(ctarget)
        b_sdata, b_starget = Variable(b_sdata), Variable(b_starget)
        # c_sdata, c_starget = Variable(c_sdata), Variable(c_starget)
        ...
        for eachbit in nbits:
            with enabler(net, wqmode, aqmode, eachbit, silent=True):
                qc_output, qb_soutput = net(cdata), net(b_sdata)
                qcloss = taskloss(qc_output, ctarget)
                qbloss = taskloss(qb_soutput, b_starget)

                tloss += const1 * (qcloss + const2 * qbloss)

                q08_closs[eachbit] = q08_closs.get(eachbit, 0.) + qcloss.data.item() * bsize
                q08_bloss[eachbit] = q08_bloss.get(eachbit, 0.) + qbloss.data.item() * bsize
    ...
    f32_closs /= dataset_size
    f32_bloss /= dataset_size
    tot_loss  /= dataset_size
    q08_closs = {b: v / dataset_size for b, v in q08_closs.items()}
    q08_bloss = {b: v / dataset_size for b, v in q08_bloss.items()}

    str_report  = f' : [epoch:{epoch}][train] [tot: {tot_loss:.4f} = '
    str_report += f'fc-xe: {f32_closs:.3f} + {const2:.2f} * fb-xe: {f32_bloss:.3f} + {const1:.2f} * ['
    for eachbit in q08_closs:
        str_report += f"('{eachbit}b' qc-xe: {q08_closs[eachbit]:.3f} + {const2:.2f} * qb-xe: {q08_bloss[eachbit]:.3f}) + "
    str_report = str_report[:-3] + ']'
    print(str_report)

    tot_lodict = {
        'fc-loss': f32_closs,
        'fb-loss': f32_bloss,
        **{f'{b}-loss': {'qc-loss': q08_closs[b], 'qb-loss': q08_bloss[b]} for b in q08_closs}
    }
...
def _compute_accuracies( \
    ...
    accuracies['fp'] = (clean_facc, clean_floss, bdoor_facc, bdoor_floss)
